    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Card className="border-l-4 border-l-blue-600">
          <p className="text-xs font-bold text-slate-500 uppercase">Registered Customer Accounts</p>
          <h3 className="text-2xl font-black text-slate-900 mt-1">{customers.length} Accounts</h3>
        </Card>

        <Card className="border-l-4 border-l-red-600">
          <p className="text-xs font-bold text-red-700 uppercase">Total Outstanding Debt Balance</p>
              ...
              {filteredCustomers.map((c) => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-3 font-bold text-slate-700">{formatCurrency(lim)}</td>
                    <td className="p-3 font-black text-red-600">{formatCurrency(bal)}</td>
                    <td className="p-3">
                      {bal >= lim ? (
      ...
      <Dialog isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title="Create Customer Account">
        <form onSubmit={handleCreate} className="space-y-3">
          ...
          <Input label="Credit Limit (KES)" type="number" value={borrowLimit} onChange={(e) => setBorrowLimit(e.target.value)} required />
      ...
      >
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3 bg-slate-900 text-white p-4 rounded-xl text-center">
            <div>
              <p className="text-[10px] text-slate-400 font-bold uppercase">Current Debt Balance</p>
              <p className="text-lg font-black text-red-400">{formatCurrency(selectedCustomer?.balance ?? selectedCustomer?.current_debt ?? 0)}</p>
            </div>
            <div>
              <p className="text-[10px] text-slate-400 font-bold uppercase">Credit Limit</p>
              <p className="text-lg font-black text-blue-400">{formatCurrency(selectedCustomer?.credit_limit ?? selectedCustomer?.borrow_limit ?? 0)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-3 border rounded-xl space-y-2 bg-slate-50">
              <h4 className="text-xs font-bold text-slate-900 flex items-center">
                <Edit3 className="w-4 h-4 mr-1 text-blue-600" /> Update Credit Limit
              </h4>
              <Input
